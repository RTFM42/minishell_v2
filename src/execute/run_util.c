...
/*   Updated: 2024/07/23 17:44:25 by yushsato         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exec_logic(t_exec *exec, char **envp)
{
	...
	if (!exec->fd_stat && !exec->is_pipe && exec->node->args
		&& isbuiltin(exec->node->args[0]))
		exec->status = exec_builtin(exec->node->args, envp, exec->ofd);
	else if (!exec->fd_stat && exec->node->args)
	{
		exec->is_promise = 1;
...
void	exec_pipe(t_exec *exec, char **envp)
{
	...
	if (exec->ofd[1] == 1)
		ft_memcpy(exec->ofd, exec->ofp, sizeof(int) * 2);
	if (!exec->fd_stat && exec->node->args)
	{
		exec->is_promise = 1;
		promise_add(async(exec->node->args, envp, exec->ifd, exec->ofd));
		if (g_signal != 0)
